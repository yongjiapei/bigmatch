(function(){
	...
	fd.write('<table width="100%" border="0" bgcolor="#CCCCCC" cellspacing="0" cellpadding="0">'+
			 ...
			 '<tr bgcolor="#FFFFFF"><td><table width="100%" border="0" bgcolor="#10C0BC" cellspacing="1" cellpadding="0">'+
			 '<tr bgcolor="#ebf4f3" height="18"><td>日</td><td>一</td><td>二</td><td>三</td><td>四</td><td>五</td><td>六</td></tr></table></td></tr>'+
	...
	function formatDay(timestr){
		...
		var str="",day=1;
		str+='<table width="100%" border="0" bgcolor="#eee" cellspacing="1" cellpadding="0">';
		...
		each.call(ids['calenderDay'].getElementsByTagName("td"),function(o,i){
			addEvent(o,"mouseover",function(e){
				o.style.backgroundColor="#10C0BC";
			})
			...
			addEvent(o,"click",function(e){
				...
				oInput.value=ids['y'].innerHTML+"-"+ids['m'].innerHTML+"-"+ fillzero(o.innerHTML) ; //只要年月日
    ...
    //获取要实现控件的表单
    function getObj_bak(className){
        var el = '',num = 0,oArr=[]
        var ishave = setInterval(function () {
            el = getElementsByTClassName(className);
            if(el){
